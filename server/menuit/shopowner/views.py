...
class ManageProducts(APIView):
    def get(self, request):
        if 'store_id' in request.query_params:
            ...
            if store:
                if 'division_name' in request.query_params:
...
class ManageProfile(APIView):
    ...
    def get(self, request):
        ...
        # user = Users.objects.get(id=request.query_params.get('user_id'))
        # if 'user_id' in request.query_params:
        ...
        return Response(serialized_shopOwner.data, status=200)
        # else:
        #     return Response(status=401)
    def post(self, request):
        try:
    ...
    def get(self, request):
        if 'store_id' in request.query_params:
            if Stores.objects.filter(id = request.query_params.get('store_id')).exists():
                store = Stores.objects.get(id = request.query_params.get('store_id'))
                serialized_store = StoreSerializer(store)
                return Response(serialized_store.data)
                
            else:
                return Response({"reason":'required store_name and shop_owner_email '},status=422)
      
        elif 'shop_owner_email' in request.query_params and 'store_name' in request.query_params :
...
class ManageDivision(APIView):
    def get(self, request):
        if 'store_id' in request.query_params:
            store = Stores.objects.filter(id = request.query_params.get('store_id'))
            print('you are here')
            if store:
                if 'division_id' in request.query_params:
                    if Division.objects.filter(id = request.query_params.get('division_id'), division_store = store ).exists():
                        divisions = Division.objects.get(id = request.query_params.get('division_id'))
                        serialized_division = DivisionSerializer(divisions, many = False)
                    else:
                        return Response(status=404)
                else:
                    divisions = Division.objects.all()
                    serialized_division = DivisionSerializer(divisions, many = True)
                    
                return Response(serialized_division.data,status=200)
