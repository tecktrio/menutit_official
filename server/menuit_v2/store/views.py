    ...
    def get(self, request):
        if 'store_id' in request.query_params:
            ...
            if store:
                ...
                elif 'division_id' in request.query_params:
...
class ManageStore(APIView):
    ...
    def get(self, request):
        ...
        if 'search' in request.query_params:
                    print('applying search filter')
                    keyword = request.query_params.get('search')
                    stores = Stores.objects.filter(store_name__startswith = keyword)
                    if stores:
                        stores = stores
                    else:
                        stores = []
                        
                    serialized_store = StoreSerializer(stores, many = True)
    ...
    def get(self, request):
        if 'store_id' in request.query_params:
            ...
            if store:
                ...
                division = Division.objects.filter(division_store=store)
                serialized_division = DivisionSerializer(division, many = True)
                # else:
